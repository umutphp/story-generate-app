...
"""
        ...
        sample_stories = models.Story.objects.filter(language='en').exclude(story_trigger__isnull=True)
        example_stories = []
        ...
        response = client.chat(model='llama3:latest', messages=[
            {
                'role': 'user',
                'content': final_prompt.format(setting=trigger.setting_str_for_prompt),
            },
        ])
